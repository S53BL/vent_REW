...
void LCD_Init(void)
{
  ...
  LCD_WriteData(LCD_ROTATION == 1 ? 0x60 : (LCD_ROTATION == 3 ? 0xA0 : 0x00)); // 0x60 for rot 1, 0xA0 for rot 3, 0x00 for 0
  LCD_WriteCommand(0x20); // Invert display off
  ...
  // Test draw: fill screen with black, then draw red rectangle
  // Fill with black
  LCD_SetCursor(0, 0, LCD_WIDTH-1, LCD_HEIGHT-1);
  for (uint32_t i = 0; i < LCD_WIDTH * LCD_HEIGHT; i++) {
    LCD_WriteData_Word(0x0000); // Black
  }
  // Draw red rectangle
  LCD_SetCursor(0, 0, LCD_WIDTH-1, 0); // Top
  for (uint16_t i = 0; i < LCD_WIDTH; i++) LCD_WriteData_Word(0xF800);
  LCD_SetCursor(0, LCD_HEIGHT-1, LCD_WIDTH-1, LCD_HEIGHT-1); // Bottom
  for (uint16_t i = 0; i < LCD_WIDTH; i++) LCD_WriteData_Word(0xF800);
  LCD_SetCursor(0, 0, 0, LCD_HEIGHT-1); // Left
  for (uint16_t i = 0; i < LCD_HEIGHT; i++) LCD_WriteData_Word(0xF800);
  LCD_SetCursor(LCD_WIDTH-1, 0, LCD_WIDTH-1, LCD_HEIGHT-1); // Right
  for (uint16_t i = 0; i < LCD_HEIGHT; i++) LCD_WriteData_Word(0xF800);
...
void Backlight_Init()
{
  ...
  ledcWrite(LEDC_CHANNEL, 255); // Full brightness initially
