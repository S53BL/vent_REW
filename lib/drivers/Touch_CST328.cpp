...
{
  WireTouch.beginTransmission(Driver_addr);
  WireTouch.write((uint8_t)(Reg_addr >> 8));
  WireTouch.write((uint8_t)Reg_addr);
  if ( WireTouch.endTransmission(true)){
    printf("The I2C transmission fails. - Touch I2C Read\r\n");
    return -1;
  }
  WireTouch.requestFrom(Driver_addr, Length);
  for (int i = 0; i < Length; i++) {
    *Reg_data++ = WireTouch.read();
  }
  return 0;
}
...
uint8_t Touch_Init(void) {
  WireTouch.begin(CST328_SDA_PIN, CST328_SCL_PIN, I2C_MASTER_FREQ_HZ);
